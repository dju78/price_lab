...
def render() -> None:
    ...
    if "All items" in I.columns:
        ...
        # but a run that set its own index or price reference period
        # rebases to a different one, and this label must say which
        # actually applies. Resolved by the engine, so the label cannot
        # drift from the rebasing the engine performed.
        index_ref = resolve_index_reference_period(res["config"].index, I.index)
        m[0].metric(f"All items, {index_ref:%b %Y} = 100", f"{I['All items'].iloc[-1]:.1f}")
